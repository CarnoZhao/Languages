...
relu(x) = max(0, x)
sigmoid(x) = 1 / (1 + CUDAnative.exp(-x))
...
function network(X, Y, layer_dims; num_iterations = 30000, batch_size = 64, learning_rate = 0.0075, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8, batched = true)
    ...
    for i in 1:num_iterations
        if batched
            ...
            V, S = backward(Y, parameters, caches, V, S, beta1, beta2, i)
            parameters = update_parameters(V, S, epsilon, parameters, learning_rate)
        end
        if i % 100 == 0
...
function main(train_x, train_y, test_x, test_y; train_size = 60000, test_size = 10000, num_iterations = 30000, batch_size = 64, learning_rate = 0.0007, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8, batched = true)
    ...
    parameters = network(train_x, train_y, layer_dims, num_iterations = num_iterations, batch_size = batch_size, learning_rate = learning_rate, beta1 = beta1, beta2 = beta2, epsilon = epsilon, batched = batched)
...
parameters = main(train_x, train_y, test_x, test_y, train_size = 600, test_size = 100, num_iterations = 500, learning_rate = 0.0007, batched = false);