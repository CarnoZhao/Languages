...
function backward(Y, parameters, caches, V, S, beta1, beta2)
    ...
    dA = (AL - Y) ./ (AL .* (1 - AL))
    for i in L:-1:1
        A, A_prev, W = caches[i], caches[i - 1], parameters[i][1]
...
function network(X, Y, layer_dims; num_iterations = 3000, batch_size = 64, learning_rate = 0.0075, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8)
    ...
    for i in 1:num_iterations
        ...
        for (mini_X, mini_Y) in batches
            ...
            V, S = backward(Y, parameters, caches, V, S)
...
function main(train_x, train_y, test_x, test_y; num_iterations = 3000, batch_size = 64, learning_rate = 0.0075, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8)
    train_x = reshape(train_x, :, size(train_x)[length(size(train_x))])
    train_y = [ifelse(i == y, 1, 0) for y in train_y for i in 0:9]
    train_y = reshape(train_y, 10, :)
    test_x = reshape(test_x, :, size(test_x)[length(size(test_x))])
    test_y = [ifelse(i == y, 1, 0) for y in test_y for i in 0:9]
    test_y = reshape(test_y, 10, :)
    layer_dims = [size(train_x)[1], 100, 50, 20, size(train_y)[1]]
